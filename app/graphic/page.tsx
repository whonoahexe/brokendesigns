...
const graphic = () => {
  ...
  const filteredWorks = Works.filter(work => work.field === "Graphic Design" && work.category === categories[category]);
        ...
        <div className="border-[0.5px] md:border-white-primary rounded-full flex w-full justify-center">
          {allFields.map((tab, index) => (
            <Link key={index} href={getFirstWordLowercase(tab!)}>
              <Button key={index} text={tab} isActive={tab === "Graphic Design"} />
            </Link>
          ))}
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
          {filteredWorks.map((work, index) => (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              transition={{ duration: 0.25, ease: [.77,0,.18,1] }}
              className={`col-span-12 lg:col-span-${work.cols}`}
            >
              <WorkCard src={work.image} alt={work.title} desc={work.description} cols={work.cols} field={work.field!} className="mt-12" />
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
